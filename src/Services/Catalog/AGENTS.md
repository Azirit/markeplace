...
## Learning Plan
- В корне проекта есть файл `LEARNING_PLAN.md` — это план самостоятельного обучения после курса
- Содержит темы, которые курс не покрывает, но необходимы для senior-уровня в микросервисах
- Используется как трекер прогресса: отмечать ✅ пройденные темы
